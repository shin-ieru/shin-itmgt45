...
def savings(gross_pay, tax_rate, expenses):
    ...
    # Let's calculate the after-tax pay first.
    after_tax_pay = gross_pay * (1-tax_rate)
    # We're asked to round it down.
    import math
    after_tax_pay_down = math.floor (after_tax_pay)
    # Subtract the expenses.
    savings = after_tax_pay_down - expenses
    return savings
...
def material_waste(total_material, material_units, num_jobs, job_consumption):
    ...
    # Replace `pass` with your code.
    # Stay within the function. Only use the parameters as input. The function should return your answer.
    # Step 1 indicates to multiply the number of jobs by the material consumption per job.
...
def interest(principal, rate, periods):
    ...
    # Replace `pass` with your code.
    # Stay within the function. Only use the parameters as input. The function should return your answer.
    # First, we should multiply the `principal` to the `quantity` to get the `simple_interest`.
    quantity = rate * periods
    simple_interest = principal * quantity
    # Now that we have gotten the interest that the investment has gotten over a number of periods, calculate the final value.
    final_value = simple_interest + principal
    import math
    interest = math.floor (final_value)
